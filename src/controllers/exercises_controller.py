...
VALID_BODYPARTS for validation of user entries.
'''
...
# Import for Integrity error handling
from sqlalchemy.exc import IntegrityError

# Import PostgreSQL error codes
from psycopg2 import errorcodes

'''Import from utils.py:
auth_as_admin_or_owner: decorator which checks if logged in user 
has authorisation to access the decorated route
...
# /exercises/body-part/<body_part> - GET - Fetch all exercises (with body_part filter)
@exercises_bp.route("/body-part/<body_part>", methods=["GET"]) 
def get_body_part_exercises(body_part):
    '''Extends fetch all exercises functionality by allowing 
    search via a filter for a specific body_part
    '''
    '''Fetch all exercises from database with filter for specified body_part mentioned in URL. 
    Capitalise the first letter when filtering to match VALID_BODYPARTS, 
    then order in alphabetical order.
    '''
    stmt = (
        db.select(Exercise)
        .filter_by(body_part=body_part.capitalize())
    ...
    # Else:
        ...
        return {"error": f"There are currently no '{body_part}' exercises that could be found. "
                f"Our current body part categories include {VALID_BODYPARTS}."
                }, 404
...
def get_user_exercises(user_id):
    ...
    if user_exists:
        ...
        # Else:
            ...
            return {"error": f"We could not find any exercises created "
                    f"by '{user_exists.username}' with ID {user_id}."
                    }, 404
...
#/exercises/user/<user_id> - GET - Fetch a filtered list of exercises created by a user.
@exercises_bp.route("/user/<int:user_id>/filter", methods=["GET"])
def filter_user_exercises(user_id):
    '''Filter by query parameter (e.g. ?body_part=Chest)'''
    ...
    if body_part:
        ...
        # Else:
            ...
            return {"error": f"'{body_part}' is an invalid body_part. "
                    f"Please search by {', '.join(VALID_BODYPARTS)}"
                    }, 400
...
def create_exercise():
    try:
        ...
        if existing_exercise: 
            return {"error": f"An exercise with the name '{existing_exercise.exercise_name}' "
                    "already exists. Please choose a different name."}, 400
...
@jwt_required() # Check if user is logged in using jwt_required
@auth_as_admin_or_owner # Validates existance of exercise id and authority of user
def delete_exercise(exercise_id):
    ...
    if exercise_is_used and not user_is_admin():
        # return error
        return {"error": f"Exercise with 'ID - {exercise_id}' is being used in existing routine/s. "
                "Delete has been aborted. If action is still required, "
                f"email admin: {ADMIN_EMAIL}"}, 409
...
def update_exercise(exercise_id):
    try:
        ...
        if exercise_is_used and not user_is_admin():
            # return error
            return {"error": f"Exercise with 'ID - {exercise_id}' is being used in an existing routine/s. "
                    "Update has been aborted. If action is still required, "
                    f"email admin: {ADMIN_EMAIL}"}, 409
